"""Send the morning brief through Gmail's SMTP endpoint.
...
def send(
    ...
    *,
    ...
    try:
        ...
        raise MailError(f"Could not send the brief via {host}:{port}: {exc}") from exc
    log.info("morning brief sent to %s", message["To"])
